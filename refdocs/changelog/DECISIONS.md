...
### ADR-016 — Embed pop-up ads can't be blocked in-page; "Pop-up Shield" reorders sources instead
**Decision:** We do **not** attempt to suppress the ad tabs that Videasy opens on play/pause. Instead, an opt-in "Pop-up Shield" setting (`sourceQueue.getPopupShield()`, off by default) reorders the failover queue so sandbox-**protected** sources (VidSrc, 2Embed — `sourceIsProtected(id) === true`, i.e. `sourceSandbox(id) !== null`) are tried before unprotected ones (Videasy), which drop to last-resort fallback. Videasy also gained a `note: "pop-up ads"` label in the source picker so the trade-off is visible without opening Settings.

**Why in-page blocking is impossible:** an unsandboxed cross-origin iframe can call `window.open()` on any user gesture, and the parent page has **no** API to intercept or cancel that — it happens in the embed's own origin. The browser's own popup blocker only applies when the iframe is *sandboxed without `allow-popups`*, but Videasy refuses to run sandboxed at all (ADR-013), so there is no configuration in which we can both play Videasy and block its popups. The only levers available are: (a) prefer sources that *do* accept the sandbox, or (b) stop embedding entirely and extract the raw stream (see `PLAN_NATIVE_PLAYER.md` — the real fix, gated on a provider spike).

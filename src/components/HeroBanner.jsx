...
// ── Constants ─────────────────────────────────────────────────────────────────
const ROTATION_INTERVAL_MS = 12_000; // 12s auto-advance
const HOVER_DELAY_MS = 2_000;        // 2s hover/visibility threshold before trailer
const LEAVE_DELAY_MS = 4_000;        // 4s after mouse-off before trailer fades

// Volume/mute icon (inline SVG so no extra icon dep needed)
const VolumeOnIcon = () => (
...
/**
 * Props:
 *   trending       – array of movie objects (from /trending/movie/week)
 *   trendingTV     – array of TV objects (from /trending/tv/week)
 *   apiKey         – TMDB API key
 *   onSelect       – fn(item) opens detail page
 *   onSave         – fn(item) toggles save status
 *   saved          – array of saved items
 */
export default function HeroBanner({
  ...
  // Build up to 5 hero candidates: interleave movie + TV
  ...
  // Per-item detail cache: { [tmdbId_type]: { tagline, trailerKey } | null }
  const detailCacheRef = useRef({});
  const [detail, setDetail] = useState(null); // resolved detail for current hero
  ...
  // Hover / visibility timers
  const hoverTimerRef = useRef(null);
  const leaveTimerRef = useRef(null);
  const rotationTimerRef = useRef(null);
  const isHoveringRef = useRef(false);

  // Check user autoplay setting (live — re-read on each render)
  ...
  // Reset trailer when hero changes
  useEffect(() => {
  ...
  const startRotation = useCallback(() => {
    ...
    rotationTimerRef.current = setInterval(() => {
      if (isHoveringRef.current || trailerActive) return; // pause while hovering / trailer active
  ...
  const isHeroSaved = useMemo(() => {
    ...
    // saved is an object, not an array - convert to array first
  ...
  return (
    ...
    >
      ...
      {/* ── Gradient overlay (always on top of backdrop/video) ── */}
      ...
      {/* ── Bottom-right controls: dots + unmute ── */}
      {trailerActive && !prefersReducedMotion ? (
        ...
        /* Normal dot indicators (poster mode) */
        heroPool.length > 1 && (

...
export default function OnePacePage({ arcs, progress, onSelectArc }) {
  ...
  // Calculate arc watch stats
  const arcStats = useMemo(() => {
    if (!arcs || !progress) return {};
    const stats = {};
    for (const arc of arcs) {
      ...
      // In DendyLusus/one-pace-map, arc episodes are nested under arc.episodes
      // But listOnePaceArcs returns a simplified metadata version. 
      // We'll read the full arc inside OnePaceArcPage, but here we can check progress key names.
      // Progress key format: onespace.{arcId}.{episodeNumber}
        ...
        {SAGAS.map(saga => {
          ...
          return (
            <button
              ...
              onMouseEnter={e => {
                if (!active) e.currentTarget.style.background = "var(--surface3)";
              }}
              onMouseLeave={e => {
            ...
            return (
              ...
              >
                ...
                <div style={{ padding: 18, flex: 1, display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
                  <div>
                    {/* Chapter & original episodes info */}

...
export default function OnePaceArcPage({ arcHeader, progress, onBack, onPlayEpisode }) {
  ...
  useEffect(() => {
    let mounted = true;
    window.electron.getOnePaceArc(arcHeader.id)
      .then(res => {
        if (!mounted) return;
        if (res?.ok) {
          setArcData(res.arc);
        } else {
          setError(res?.error || "Failed to load arc data");
        }
      })
      .catch(err => {
        if (!mounted) return;
  ...
  return (
    <div style={{ padding: "30px 40px", color: "var(--text)" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "300px 1fr", gap: 40 }} className="onepace-details-grid">
        {/* Left Side: Arc Info Panel */}
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          {/* Stylized Poster Frame */}
          <div
            style={{
        ...
        <div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            {episodes.map(ep => {
              ...
              if (ep.releaseDate) {
                try {
                  formattedDate = new Date(ep.releaseDate).toLocaleDateString(undefined, {
                    year: "numeric",
                    month: "long",
                    day: "numeric"
                  });
                } catch {
                  formattedDate = ep.releaseDate;
                }
              }
                ...
                >
                  ...
                  <div style={{ padding: 18, flex: 1, minWidth: 0, display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
                    <div>
                      ...
                      {/* Chapters / date / duration details */}
                      <div style={{ display: "flex", gap: 14, fontSize: 11, color: "var(--text3)", marginBottom: 10 }}>
                        {ep.mangaChapters && (
                          <span>📖 Chapters: <strong style={{ color: "var(--text2)" }}>{ep.mangaChapters}</strong></span>
                        )}

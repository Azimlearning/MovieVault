...
export default function App() {
  ...
  useEffect(() => {
    ...
    window.electron.getAppVersion().then((version) => {
      const lastVersion = localStorage.getItem("movievault_lastVersion");
      if (lastVersion && lastVersion !== version) {
        clearAppCaches();
      }
      localStorage.setItem("movievault_lastVersion", version);
    });
  }, []);
  ...
  useEffect(() => {
    ...
    window.addEventListener("movievault:library-sort-changed", handler);
    return () =>
      window.removeEventListener("movievault:library-sort-changed", handler);
  }, []);

  // ── Re-fetch trending immediately when metadata language changes ──────────
  useEffect(() => {
    const handler = () => fetchTrending();
    window.addEventListener("movievault:tmdb-lang-changed", handler);
    return () =>
      window.removeEventListener("movievault:tmdb-lang-changed", handler);
  }, [fetchTrending]);
  useEffect(() => {
    // Accent colour
    ...
    <ErrorBoundary>
      ...
      <div>
        ...
        {updateBanner && (
          ...
          >
            <span>🎉 MovieVault v{updateBanner.latest} is available!</span>
            <button
